...
def AdminHome(request):
    ...
    pending_teams = TeamDetails.objects.filter(
        team_status='not approved').count()
    approved_teams = TeamDetails.objects.filter(team_status='approved').count()
    ongoing_tournaments = TournamentDetails.objects.filter(
        tournament_status='not completed')
...
def AddSport(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            do_exist = SportsDetail.objects.filter(
                sport_name=sport_name.lower(), sport_type=sport_type).exists()
            if do_exist:
                error_msg = "Sport Already Added"
                return render(request, 'Admin/AddSports.html', {'form': form, 'error_msg': error_msg, 'sports': sports_data})
            else:
                qry = SportsDetail(
                    sport_name=sport_name.lower(), sport_type=sport_type)
                qry.save()
...
def AddNotification(request):
    ...
    if request.method == 'POST':
        ...
        qry = AdminNotification(notfn_title=notfn_title,
                                notfn_date=st_dt, notfn_content=notfn_text)
        qry.save()
...
def AddTournament(request):
    ...
    if request.method == 'POST':
        ...
        venue_id = VenueDetails.objects.get(
            venue_id=request.POST['venue_list'])
        ...
        if st_con < cur_date:
            ...
            reg_dt = str(reg_con.day)+'/'+str(reg_con.month) + \
                '/'+str(reg_con.year)

            qry = TournamentDetails(tournament_title=tournament_title, venue_id=venue_id, sport_type=sport_id, players_allowed=players_allowed,
                                    start_date=st_dt, end_date=en_dt, reg_date=reg_dt, reg_fee=reg_fee, win_prize=win_prize, tournament_status="not completed")
            qry.save()
...
def getVenue(request):
    ...
    print(sport_name)
    venue_list = VenueDetails.objects.filter(
        Q(venue_type=sport_name) | Q(venue_type="both"))
...
def TournamentRequest(request):
    ...
    t_data = TournamentDetails.objects.filter(
        tournament_status="not completed")
...
def ViewTournamentPlayers(request):
    ...
    players_list = TournamentPlayers.objects.filter(
        team_id=request.session['t'], tournament_id=request.session['tr'])
...
def UpdatePlayerStatus(request):
    ...
    if 'approve' in request.POST:
        record = TournamentPlayers.objects.get(
            player_id=player, tournament_id=request.session['tr'])
...
def GenerateFixture(request):
    tr_data = TournamentDetails.objects.filter(
        tournament_status='not completed')
    dt_dict = {}
    dt_arr = []
    fixture_data = ""
    if request.method == 'POST':
        ...
        if 'upd' in request.POST:
            ...
            tr = TournamentRegistration.objects.filter(
                tournament_id=request.POST['tr'])
            dt = TournamentDetails.objects.get(
                tournament_id=request.POST['tr'])
            ...
            for x in range(0, diff.days+1):
                ...
                dt_dict['date'] = str(st.day)+'/' + \
                    str(st.month)+'/'+str(st.year)
                dt_arr.append(dt_dict)
                dt_dict = {}
            print(dt_arr)
            fxt = FixtureGenerate(tr, tr.count())

            fixture_generated = Fixture.objects.filter(
                tournament_id=request.POST['tr']).exists()

            if not fixture_generated:
                for i in fxt:

                    fixture = Fixture(
                        tournament_id=dt, team1=i['t1'], team2=i['t2'], match=i['match'])
                    fixture.save()

                # print(i['t1'])

            fixture_data = Fixture.objects.filter(
                tournament_id=request.POST['tr'])
...
def UpdateFixture(request, id):
    ...
    if request.method == 'POST':
        if 'btn-leg' in request.POST:
            ...
            Fixture.objects.filter(tournament_id=id, match=type).update(
                match=m, date=date, time=time, team1=team1, team2=team2)
...
def MatchStatus(request):
    ...
    if request.method == 'POST':
        ...
        tr_reg = TournamentRegistration.objects.get(
            tournament_id=request.POST['tr'], team_id=wonby)
        tr_reg.team_point += 1
        tr_reg.save()
        ...
        if fixture.id == is_final.id:
            data = TournamentDetails.objects.get(
                tournament_id=request.POST['tr'])
...
def LoadSemi(request):
    ...
    set2 = []
    count = 1
    data_set = TournamentRegistration.objects.filter(
        tournament_id=id).order_by('-team_point')[:4]
